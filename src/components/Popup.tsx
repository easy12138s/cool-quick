...
const Popup = memo<PopupProps>(
  ({ content, contentType, autoCloseSeconds, sourceApp, onSave, onDismiss }) => {
    ...
    const displayContent =
      isPassword && !showPassword ? maskPassword(content) : truncateContent(content)
    ...
    return (
      <div className="w-full h-full flex items-center justify-center p-4 bg-transparent">
        ...
        >
          ...
          <div
            className={`relative px-5 py-4 bg-gradient-to-r ${config.gradient} text-white overflow-hidden`}
          >
          ...
          <div className="p-5">
            {/* Content display */}
            <motion.div
              className="relative bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-4 border border-slate-100 dark:border-slate-700/50 group max-h-[120px] overflow-y-auto scrollbar-thin"
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.1 }}
            >
              <p
                className={`text-sm ${isPassword ? 'font-mono tracking-wider' : 'font-mono'} text-slate-800 dark:text-slate-200 break-all leading-relaxed`}
              >
                {displayContent}
              </p>
            ...
            >
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-slate-400">{isHovered ? '已暂停' : '秒后关闭'}</span>
              </div>
            ...
            >
              <span className="text-[10px] text-slate-400 flex items-center gap-1">
                <kbd className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 rounded text-slate-500 font-mono">
                  Enter
                </kbd>
                保存
              </span>
